...
const CadastrarUsuario = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: adminProfile
      } = (await supabase.from("profiles" as any).select("id_clinica").eq("id", user.id).single()) as {
        data: any;
      };
      ...
      toast.success(`Usuário ${validated.nome} cadastrado com sucesso! Senha padrão: ${senhasPadrao}`);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="nome" placeholder="Digite o nome completo" value={formData.nome} onChange={e => handleChange("nome", e.target.value)} required className="mt-1" />
                </div>
